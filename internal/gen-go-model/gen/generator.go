...
import (
    ...
    "github.com/jspc/dbml-go/internal/gen-go-model/genutil"
)
...
func (g *generator) headerComments() string {
    return fmt.Sprintf(`Generated by github.com/jspc/dbml-gen-go-model
    version: %s
    timestamp: %s
...
func (g *generator) genEnum(enum core.Enum) error {
    ...
    f.Func().Params(
        ...
        jen.If(jen.Op("!").Id("ok").Block(
            jen.Return(jen.Qual("fmt", "Errorf").Call(jen.List(jen.Lit("value (%#v) is not of the expected type []uint8"), jen.Id("value")))),
        )),
        jen.Switch().BlockFunc(func(group *jen.Group) {
            ...
            group.Default().Block(
                jen.Err().Op("=").Qual("fmt", "Errorf").Call(jen.List(jen.Lit(fmt.Sprintf("unable to parse %%#v into a known %s type", enumGoTypeName)), jen.Id("value"))),
            )
        }),
        jen.Return(),
    )
